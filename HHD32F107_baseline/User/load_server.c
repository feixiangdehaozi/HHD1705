...
//    口操作位宽，服务器返回一次能接收的最大数据长度和spi操作位宽					  //
// 2、客户端根据获取到的最大缓冲信息，计算数据的发送次数，将该次数通过"fileinfo:"xxxx //
//    反馈给服务器，同时客户端保证数据按操作位宽对其								  //
// 3、服务器接收数据时，会检擦帧序号是否连续，不连续会返回给客户端错误，并指明期望的帧//
//    的序号；服务器将接收的数据通过SPI发送完成后，才会反馈客户端接收完成，客户端必须 //
//    在收到应答后再发送下一帧数据													  //
...
int info_deal(uint8_t *data)
{
	...
	{
		((uint16_t *)data)[0] = MAX_PACKAGE_LEN-10;	//缓冲区大小
...
int frame_handler(uint8_t *data)
{
	...
	{
		writeArray(data, 4);				//提示接收端，放弃已接收的数据
		frame_count = 0;
		totaFrameCount = 0;
		return -2;
	}
...
static err_t tcp_echoserver_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
  ...
  else if((es->state == ES_RECEIVED) || (es->state == ES_ACCEPTED) )
  {
	...
	////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			ret = frame_handler(data_buff);						  //处理下发数据
			if(ret > 0)
			{
				if(ret >= 5)
				{
