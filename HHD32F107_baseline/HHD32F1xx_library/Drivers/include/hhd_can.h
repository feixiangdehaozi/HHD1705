...
uint32_t DIRECTION:
    1;             //If ??1?бе, the error occurred during reception. If ??0?бе, the error occurred during transmission.
uint32_t ERRORCODE:
...
{
uint32_t DLC:
    4;               //Specify the number of data bytes included in the message being sent.
...
typedef struct
{
    ...
    __IO   union CAN_ALC_REG
        ALC;          /*!< Offset: 0x02C Arbitration Lost Capture Register    (R) */
    ...
    __I    union CAN_RBSA_REG
        RBSA;         /*!< Offset: 0x078 Receive Buffer Start Address Register    (R) */
