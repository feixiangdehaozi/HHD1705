...
uint32_t LDOK:
    1;	//This R/W bit loads the prescaler bits of CTRL and the entire CMOD and VALm registers into a set of buffers. The buffered prescaler divisor, PWM counter modulus value, and PWM pulse width take effect at the next PWM reload. Set LDOK by reading it, then write a Logic 1 to it. LDOK is automatically cleared after the new values are loaded, or can be manually cleared before a reload by writing a Logic 0 to it.
uint32_t PWMF:
    1;	//This R/W flag is set at the beginning of every reload cycle regardless of the state of the LDOK bit. Clear PWMF by reading it, and then write a Logic 0 to it. If another reload occurs before the clearing sequence is complete, writing Logic 0 to the PWMF has no effect.
    uint32_t PWMRIE: 1;	//This R/W bit enables the PWMF flag to generate interrupt request.
    uint32_t RSVD0: 1;
    uint32_t PRSC: 2;	//WM Clock Frequency.
uint32_t IPOL0:
    1;	//Select the PWM value register for the PWM_0 and PWM_1 pins pair in complementary mode.
uint32_t IPOL1:
    1;	//Select the PWM value register for the PWM_2 and PWM_3 pins pair in complementary mode.
uint32_t IPOL2:
    1;	//Select the PWM value register for the PWM_4 and PWM_5 pins pair in complementary mode.
    uint32_t RSVD1: 1;
uint32_t HALF:
    1;	//Enable half-cycle reloads in Center-Aligned PWM mode. This bit has no effect on Edge-Aligned PWMs.
uint32_t LDFQ:
    4;	//These buffered R/W bits select the PWM load frequency. Reset clears the LDFQ bit field, selecting loading every PWM opportunity. The occurrence of a PWM opportunity is determined by the half bit.
    ...
    uint32_t RSVD2: 2;
uint32_t TSCNT:
    2;	//Trig signal output count register. This field can determine the interval between each output trig signal
uint32_t TSSEL:
    2;	//Trig signal select register. The output trig signal will be send to ADC and Timer.
...
struct PWM_FLTACK_BITS
{
    ...
    uint32_t RSVD0: 13;
uint32_t FFLAG0:
    1;	//These read-only flags are set within two PWM Operation Clock cycles after a rising edge on the PWM_FAULT0 pin. Clear FFLAG0 by writing a Logic 1 to the FTACK0 bits in the FLTACK register.
uint32_t FPIN0:
    1;	//These read-only bits reflect the current state of the filtered PWM_FAULT pin. A reset has no effect on FPIN0
uint32_t FFLAG1:
    1;	//These flags are set by software. Clear FFLAG6 by writing a Logic 1 to the FTACK6 bits in the FLTACK register.
    uint32_t RSVD1: 1;
uint32_t FFLAG2:
    1;	//This read-only flags are set within two PWM Operation Clock cycles after the ADC limit l fault happened. Clear FFLAG7 by writing a Logic 1 to the FTACK7 bits in the FLTACK register.
...
uint32_t NDEP01:
    1;	//This write-protectable bit determines if the PWM Channel 0-1 will be independent PWMs or complementary PWM pairs.
uint32_t NDEP23:
    1;	//This write-protectable bit determines if the PWM Channel 2-3 will be independent PWMs or complementary PWM pairs.
uint32_t NDEP45:
    1;	//This write-protectable bit determines if the PWM Channel 4-5 will be independent PWMs or complementary PWM pairs.
    uint32_t RSVD0: 1;
uint32_t BOTNEG01:
    1;	//This write-protectable bit determines the polarity for the bottom-side PWMs.
uint32_t BOTNEG23:
    1;	//This write-protectable bit determines the polarity for the bottom-side PWMs.
uint32_t BOTNEG45:
    1;	//This write-protectable bit determines the polarity for the bottom-side PWMs.
    ...
    uint32_t RSVD2: 1;
uint32_t EDG:
    1;	//his write-protectable bit determines whether all PWM channels will use Edge-Aligned or Center-Aligned wave forms.
    uint32_t RSVD3: 17;
};
union PWM_CNFG_REG
{
...
struct PWM_CCTRL_BITS
{
    ...
    uint32_t RSVD0: 1;
uint32_t VLMODE:
    2;	//These two bits determine the way the Value registers are being loaded. These bits are write-protected when ENHA is zero.
...
struct PWM_PORT_BITS
{
    ...
    uint32_t RSVD1: 2;
uint32_t FPSEL0:
    1;	//When fault happens, this bit selects whether FAULTVAL0 controlling PWM output or setting PWM0 pin to input mode
uint32_t FPSEL1	:
    1;	//When fault happens, this bit selects whether FAULTVAL1 controlling PWM1 output or setting PWM1 pin to input mode
uint32_t FPSEL2:
    1;	//When fault happens, this bit selects whether FAULTVAL2 controlling PWM2 output or setting PWM2 pin to input mode
uint32_t FPSEL3	:
    1;	//When fault happen, this bit selects whether FAULTVAL3 controlling PWM3 output or setting PWM3 pin to input mode
uint32_t FPSEL4:
    1;	//When fault happens, this bit selects whether FAULTVAL4 controlling PWM4 output or setting PWM4 pin to input mode
uint32_t FPSEL5:
    1;	//When fault happens, this bit selects whether FAULTVAL5 controlling PWM output or setting PWM5 pin to input mode
    uint32_t RSVD2: 10;
};
union PWM_PORT_REG
{
...
struct PWM_ICCTRL_BITS
{
    ...
    uint32_t RSVD0: 1;
uint32_t PAD_EN:
    1;	//The PWM_m output pads can be enabled or disabled by setting the PAD_EN bit. The power-up default has the pads disabled. This bit does not affect the functionality of the PWM, so the PWM can be energized with the output pads disabled. This enable is to power-up with a safe default value for the PWM drivers.
    uint32_t RSVD1: 27;
};
...
{
    __IO union PWM_CTRL_REG
        CTRL;                              /*!< Control Register                                                      */
    __IO union PWM_FCTRL_REG
        FCTRL;                             /*!< Fault Control Register                                                */
    __IO union PWM_FLTACK_REG
        FLTACK;                            /*!< Fault Status/Acknowledge Register                                     */
    __IO union PWM_OUTCTRL_REG
        OUTCTRL;                           /*!< Output Control Register                                               */
    __I  union PWM_CNTR_REG
        CNTR;                              /*!< Counter Register                                                      */
    __IO union PWM_CMOD_REG
        CMOD;                              /*!< Counter Modulo Register                                               */
    __IO union PWM_VAL_REG
    ...
    __IO union PWM_DTIM_REG
        DTIM0;                             /*!< Deadtime Register 0                                                   */
    __IO union PWM_DTIM_REG
        DTIM1;                             /*!< Deadtime Register 1                                                   */
    __IO union PWM_DMAP0_REG
        DMAP0;                             /*!< Disable Mapping Register0                                             */
    __IO union PWM_DMAP1_REG
        DMAP1;                             /*!< Disable Mapping Register1                                             */
    __IO union PWM_CNFG_REG
        CNFG;                              /*!< Configure Register                                                    */
    __IO union PWM_CCTRL_REG
        CCTRL;                             /*!< Channel Control Register                                              */
    __IO union PWM_PORT_REG
        PORT;                              /*!< Port Register                                                         */
    __IO union PWM_ICCTRL_REG
        ICCTRL;                            /*!< Internal Correction Control Register                                  */
    __I  uint32_t  RESERVED1[2];
    __IO union PWM_PSCR_REG
        PSCR;                              /*!< Polarity Invert control Register                                      */
    __O  union PWM_CNTRINI_REG
        CNTRINI;                           /*!< Counter Init Register                                                 */
} HHD32F1_PWM_TypeDef;
