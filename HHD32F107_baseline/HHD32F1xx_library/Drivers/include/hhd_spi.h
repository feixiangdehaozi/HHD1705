...
uint32_t BSY:
    1;			//Busy. This bit is 0 if the SPI controller is idle, or 1 if it is currently sending/receiving a frame and/or the Tx FIFO is not empty.
    uint32_t RSVD: 27;	//
};
union SPI_SR_REG
{
...
{
uint32_t RORMIS:
    1;			//This bit is 1 if another frame was completely received while the RxFIFO was full, and this interrupt is enabled.
uint32_t RTMIS:
    1;			//This bit is 1 if the Rx FIFO is not empty, has not been read for a “time-out period? and this interrupt is enabled. The time-out period is the same for master and slave modes and is determined by the SPI bit rate: 32 bits at PCLK / (CPSDVSR ?[SCR+1]).
uint32_t RXMIS:
    1;			//This bit is 1 if the Rx FIFO is at least half full, and this interrupt is enabled.
uint32_t TXMIS:
    1;			//This bit is 1 if the Tx FIFO is at least half empty, and this interrupt is enabled.
    uint32_t RSVD: 28;	//
};
union SPI_MIS_REG
{
...
{
uint32_t RORIC:
    1;			//Writing a 1 to this bit clears the “frame was received when Rx FIFO was full?interrupt.
uint32_t RTIC:
    1;			//Writing a 1 to this bit clears the “Rx FIFO was not empty and has not been read for a time-out period?interrupt.
    uint32_t RSVD: 30;	//
};
union SPI_ICR_REG
{
...
{
    __IO union SPI_CR0_REG
        CR0;                               /*!< SPI control register0                                                 */
    __IO union SPI_CR1_REG
        CR1;                               /*!< SPI control register1                                                 */
    __IO union SPI_DR_REG
        DR;                                /*!< SPI data register                                                     */
    __I  union SPI_SR_REG
        SR;                                /*!< SPI status register                                                   */
    __IO union SPI_CPSR_REG
        CPSR;                              /*!< SPI Clock Prescale Register                                           */
    __IO union SPI_IMSC_REG
        IMSC;                              /*!< Interrupt Mask Set and Clear Register                                 */
    __I  union SPI_RIS_REG
        RIS;                               /*!< Raw Interrupt Status Register                                         */
    __I  union SPI_MIS_REG
        MIS;                               /*!< Masked Interrupt Status Register                                      */
    __O  union SPI_ICR_REG
        ICR;                               /*!< SPI Interrupt Clear Register                                          */
} HHD32F1_SPI_TypeDef;
