...
{
uint32_t MR0I:
    1; 	//Interrupt on MR0: an interrupt is generated when MR0 matches the value in the TC.
...
uint32_t MR1I:
    1; 	//Interrupt on MR1: an interrupt is generated when MR1 matches the value in the TC.
    uint32_t MR1R: 1; 	//Reset on MR1: the TC will be reset if MR1 matches it.
uint32_t MR1S:
    1; 	//Stop on MR1: the TC and PC will be stopped and TCR[0] will be set to 0 if MR1 matches the TC.
uint32_t MR2I:
    1; 	//Interrupt on MR2: an interrupt is generated when MR2 matches the value in the TC.
    uint32_t MR2R: 1; 	//Reset on MR2: the TC will be reset if MR2 matches it.
uint32_t MR2S:
    1; 	//Stop on MR2: the TC and PC will be stopped and TCR[0] will be set to 0 if MR2 matches the TC.
uint32_t MR3I:
    1; 	//Interrupt on MR3: an interrupt is generated when MR3 matches the value in the TC.
    uint32_t MR3R: 1; 	//Reset on MR3: the TC will be reset if MR3 matches it.
uint32_t MR3S:
    1; 	//Stop on MR3: the TC and PC will be stopped and TCR[0] will be set to 0 if MR3 matches the TC.
...
{
uint32_t CAP0RE:
    1; 	//Capture on CT16Bn_CAP0 rising edge: a sequence of 0 then 1 on CT16Bn_CAP0 will cause CR0 to be loaded with the contents of TC.
uint32_t CAP0FE:
    1; 	//Capture on CT16Bn_CAP0 falling edge: a sequence of 1 then 0 on CT16Bn_CAP0 will cause CR0 to be loaded with the contents of TC.
uint32_t CAP0I:
    1; 	//Interrupt on CT16Bn_CAP0 event: a CR0 load due to a CT16Bn_CAP0 event will generate an interrupt.
uint32_t CAP1RE:
    1; 	//Capture on CT16Bn_CAP1 rising edge: a sequence of 0 then 1 on CT16Bn_CAP1 will cause CR1 to be loaded with the contents of TC.
uint32_t CAP1FE:
    1; 	//Capture on CT16Bn_CAP1 falling edge: a sequence of 1 then 0 on CT16Bn_CAP1 will cause CR1 to be loaded with the contents of TC.
uint32_t CAP1I:
    1; 	//Interrupt on CT16Bn_CAP1 event: a CR1 load due to a CT16Bn_CAP1 event will generate an interrupt.
uint32_t CAP2RE:
    1; 	//Capture on comparator n level output - rising edge: a sequence of 0 then 1 on the comparator n output will cause CR2 to be loaded with the contents of TC.
uint32_t CAP2FE:
    1; 	//Capture on comparator n level output - falling edge: a sequence of 1 then 0 on comparator n output will cause CR2 to be loaded with the contents of TC.
uint32_t CAP2I:
    1; 	//Interrupt on comparator n level output event: a CR2 load due to a comparator 0 event will generate an interrupt.
uint32_t CAP3RE:
    1; 	//Capture on comparator n edge output - rising edge: a sequence of 0 then 1 on the comparator n output will cause CR3 to be loaded with the contents of TC.
uint32_t CAP3FE:
    1; 	//Capture on comparator n edge output - falling edge: a sequence of 1 then 0 on comparator n output will cause CR3 to be loaded with the contents of TC.
uint32_t CAP3I:
    1; 	//Interrupt on comparator n edge output event: a CR3 load due to a comparator n event will generate an interrupt.
...
uint32_t ENCC:
    1; 	//Setting this bit to 1 enables clearing of the timer and the prescaler when the capture-edge event specified in bits 7:4 occurs.
uint32_t SELCC:
    4; 	//When bit 3 is a 1, these bits select which capture input edge will cause the timer and prescaler to be cleared. These bits have no effect when bit 3 is low.
...
{
    __IO union TIM_IR_REG
        IR;                                /*!< Interrupt register                                                    */
    __IO union TIM_TCR_REG
        TCR;                               /*!< Timer control register                                                */
    __IO union TIM_TC_REG
        TC;                                /*!< Timer counter register                                                */
    __IO union TIM_PR_REG
        PR;                                /*!< Prescale register                                                     */
    __IO union TIM_PC_REG
        PC;                                /*!< Prescale Counter register                                             */
    __IO union TIM_MCR_REG
        MCR;                               /*!< Match Control Register                                                */
    __IO union TIM_MR_REG
        MR0;                               /*!< Match register0                                                       */
    __IO union TIM_MR_REG
        MR1;                               /*!< Match register1                                                       */
    __IO union TIM_MR_REG
    ...
    __IO union TIM_MR_REG
        MR3;                               /*!< Match register3                                                       */
    __IO union TIM_CCR_REG
        CCR;                               /*!< Capture Control Register                                              */
    __I  union TIM_CR_REG
        CR0;                               /*!< Capture register0                                                     */
    __I  union TIM_CR_REG
        CR1;                               /*!< Capture register1                                                     */
    __I  union TIM_CR_REG
        CR2;                               /*!< Capture register2                                                     */
    __I  union TIM_CR_REG
        CR3;                               /*!< Capture register3                                                     */
    __IO union TIM_EMR_REG
        EMR;                               /*!< External match register                                               */
    __I  uint32_t  RESERVED0[12];
    __IO union TIM_CTCR_REG
